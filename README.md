...
| **Supported styles** | Hand-Drawn Infographic · Halftone Paper Collage |
...
- **Style routing** chooses between continuous hand-drawn expression and modular paper assembly based on the medium the suite needs, not on topic labels or whether a person appears.
...
Optional controls: publishing platform, language, aspect ratio, number of visuals, and style (`Hand-Drawn Infographic` or `Halftone Paper Collage`).
...
Two: Hand-Drawn Infographic, and Halftone Paper Collage. Hand-drawn visuals may use objects, annotations, flows, spaces, or people when the content calls for them; people are not required. Other styles are not silently simulated.
