...
**Social Content Kit 默认不会自动**生成最终图片、自动发布，或编排动态/视频资产。每份 `figure-spec` 都是自包含的。当前会话存在可实际调用的生图工具时，Skill 可以在交付并核查规格后，询问是否继续生成图片；它不会根据平台名称猜测能力，也不会在用户确认前调用工具。
...
| 要把项目或工具整理成面向采用决策的推荐 | 只需要直接成图，不需要社媒文案或配图规格 |
...
默认不会自动生成。它先交付可直接进入视觉生产的 `figure-spec`；如果当前会话存在可调用的生图工具，且用户没有要求“仅输出规格”，则可以在交付后用本次产物的语言询问是否把成图作为经确认的下游续步。生图与读取位图会分别判断：当前会话无法检查成图时，询问会提前披露这一限制，生成结果也会标记为待完成 Bitmap QA。工具调用成功不等于图片已经验收。
