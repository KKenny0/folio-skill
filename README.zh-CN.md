...
Signal Pack 将它们视为同一个编辑问题：从 **evidence（证据）** 到 **narrative（叙事）**，再到 **visual direction（视觉方向）**。最终的文章、发布信息和配图规格共享同一个事实边界与观点，而不是三份互不相干的交付物。
...
Signal Pack 有统一的 taste，但不是固定模板。
