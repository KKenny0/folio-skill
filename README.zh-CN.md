...
**Folio** 是一个 [Agent Skill](https://github.com/KKenny0/folio-skill)，面向「来源 → 社交媒体内容套件」的编辑流程。输入 **URL**、**文件** 或 **知识点**，默认输出彼此一致的平台发布文案和自包含配图制作单；明确要求文章时才增加 `article-draft`，长文、短文或极短文只决定篇幅。
...
Folio 把它们当作同一个编辑问题：先建立内部的经核查内容核心，再从同一个事实边界派生发布文案和视觉规格。文章不再是强制中间产物，只在明确请求时生成。
...
**Folio 不负责**：生成最终图片、自动发布，或编排动态/视频资产。每份 `figure-spec` 都是自包含的：把其中的完整生图指令贴进 Codex 的 `imagegen`、ChatGPT 配图能力或其他生图工具即可。
...
$folio https://example.com/post -- 完整内容套件，包含文章和 4 张配图规格
```
...
只需摘要或普通单篇文章时，直接向 Agent 提要求即可，不必调用 Folio。
...
可以。在显式 `$folio` 请求中只要求一篇独立文章时，只返回 `article-draft`。需要三个产物时，请明确要求文章与配图、完整内容套件或公众号长文套件。

**没有可见文章时，怎样保证事实一致？**

Folio 会先在内部建立经核查内容核心，记录来源、主张、精确值、限制和不确定性，再从中派生所有用户产物。
